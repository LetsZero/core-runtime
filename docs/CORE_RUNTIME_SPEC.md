...
The _only_ real data container. Tensors are treated as first-class primitives, not library objects.
...
#### **7.1 Activation Functions (v1.1)**

Non-linear activation functions required for neural network expressiveness.

| Op        | Definition                                | Invariants                 |
| --------- | ----------------------------------------- | -------------------------- |
| `relu`    | `max(0, x)`                               | Shape-preserving, F32 only |
| `sigmoid` | `1 / (1 + exp(-x))`                       | Shape-preserving, F32 only |
| `tanh`    | `(exp(x) - exp(-x)) / (exp(x) + exp(-x))` | Shape-preserving, F32 only |

**Rules:**

- Input shape == Output shape (no broadcasting)
- F32 dtype only (multi-dtype is IR/LLVM responsibility)
- CPU-only in core (GPU is backend responsibility)
- No numerical stability tricks (stability is lowering responsibility)

> **Note:** Activations define _non-linearity_, not _probabilistic interpretation_.
> Therefore: ❌ `softmax` is NOT in core (it's reduction + normalization → belongs in IR/frontend).
